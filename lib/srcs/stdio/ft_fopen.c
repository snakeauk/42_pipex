...
/*   Updated: 2024/10/14 11:56:47 by kinamura         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_fopen(char *filename, const char *mode)
{
	...
	if (ft_strncmp(mode, "w", 1) == 0 && mode_len == 1)
		fd = open(filename, O_WRONLY | O_CREAT | O_TRUNC, 0644);
	else if (ft_strncmp(mode, "r", 1) == 0 && mode_len == 1)
		fd = open(filename, O_RDONLY);
	else if (ft_strncmp(mode, "wr", 2) == 0 && mode_len == 2)
		fd = open(filename, O_RDWR | O_CREAT | O_TRUNC, 0644);
	else if (ft_strncmp(mode, "a", 1) == 0 && mode_len == 1)
